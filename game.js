...
function launchBonusRound(type) {
  ...
  bonusHelp.textContent = type === 'shield'
    ? 'Drag around the core to move the curved shield. Block spikes for 15 seconds.'
    : 'Swipe down on each tube to dump pressure to zero. Multi-touch helps near the end.';
...
function bonusLoop(now) {
  ...
  bonusTimer.textContent = `${remain.toFixed(1)}s`;
...
function spawnDebris(rect, elapsed) {
  ...
  const speed = 135 + elapsed * 14 + Math.random() * 40;
  bonus.debris.push({
    x, y,
    vx: dx / len * speed,
    vy: dy / len * speed,
    curve: (Math.random() - 0.5) * (elapsed > 7 ? 130 : 55),
    r: 6 + Math.random() * 4,
...
function updateShieldBonus(dt, elapsed) {
  ...
  const spawnRate = Math.max(0.12, 0.8 - elapsed * 0.05);

  bonus.debrisSpawn -= dt;
  while (bonus.debrisSpawn <= 0) {
    spawnDebris(rect, elapsed);
    if (elapsed > 10 && Math.random() < 0.55) spawnDebris(rect, elapsed);
  ...
  ctx.shadowBlur = 18;
  ctx.beginPath();
  ctx.arc(0, 0, shieldR, bonus.shieldAngle - 0.48, bonus.shieldAngle + 0.48);
  ctx.stroke();
  ctx.restore();
  ...
  for (let i = bonus.debris.length - 1; i >= 0; i--) {
    ...
    const shieldHit = dist < shieldR + 14 && dist > shieldR - 22 && Math.abs(angleDiff(angle, bonus.shieldAngle)) < 0.58;
    if (shieldHit) {
