...
function saveScores(scores) {
  localStorage.setItem(SCOREBOARD_KEY, JSON.stringify(scores.slice(0, 20)));
}
...
function renderScores(autoScroll = false) {
  const scores = loadScores().sort((a, b) => b.score - a.score || b.date - a.date).slice(0, 20);
  if (!scores.length) {
    scoresList.innerHTML = '<p class="empty-scores">No scores saved yet. Be the first cell whacker.</p>';
  ...
  scoresList.innerHTML = scores.map((entry, index) => {
    ...
    return `<div class="score-row"><span class="rank">#${index + 1}</span><strong>${escapeHtml(entry.name || 'Player')}</strong><span>${entry.score}${bonusText}</span><em>${when}</em></div>`;
  }).join('');

  scoresList.classList.toggle('auto-scroll', autoScroll && scores.length > 5);
}

function showScores(autoScroll = false) {
  renderScores(autoScroll);
...
function updateFinalBreakdown() {
  ...
  const whackPoints = Math.max(0, total - bonusPoints);
  ...
  if (finalTotalScore) finalTotalScore.textContent = String(total);
}
...
function submitFinalScore(event) {
  ...
  scores.push({ name: cleanName, score: state.score, bonus: state.bonusScore, level: state.level, date: Date.now() });
...
function launchBonusRound(type) {
  ...
  bonusHelp.textContent = type === 'shield'
    ? 'Drag around the core to move the curved shield. Block spikes for 15 seconds. Bonus points are doubled and added to your score.'
    : 'Swipe down on each tube to dump pressure to zero. Bonus points are doubled and added to your score.';
...
function finishBonus(success = true, reason = '') {
  ...
  const details = `${title}  Bonus ${base} × 2 = +${award} points`;
  if (bonusResult) {
    bonusResult.textContent = details;
    bonusResult.classList.add('show');
  }
  bonusHelp.textContent = reason || details;
  showLevelBanner(success ? `DOUBLE BONUS! +${award}` : `BONUS FAILED! +${award}`, 1800);
...
function bonusLoop(now) {
  ...
  bonusTimer.textContent = `${remain.toFixed(1)}s · base ${Math.floor(bonus.basePoints)} ×2`;
